...
!------------------------------------------------------------------------------------
  ...
  subroutine print_cfl(elem,hybrid,nets,nete,dtnu,ptop,pmid,&
    ...
    if (ptop>1000.0_r8) then
      ...
      !
      top_032_042km = .true.
    else if (ptop>1e-1._r8) then
      !
      ! CAM7 top (~4.35e-1 Pa)
      !
      top_042_090km = .true.
    else if (ptop>1E-4._r8) then
      !
      ! WACCM top (~4.5e-4 Pa)
      !
      top_090_140km = .true.
...
!$OMP BARRIER
  ...
  subroutine automatically_set_viscosity_coefficients(hybrid,ne,max_min_dx,min_min_dx,nu,factor,str)
    ...
    character(len=*), parameter :: subname = 'SE dycore: automatically_set_viscosity_coefficients'
    ...
    if (nu < 0) then
      if (ne <= 0) then
        if (hypervis_power/=0) then
          call endrun(subname//': ERROR: Automatic scaling of scalar viscosity not implemented')
        else if (hypervis_scaling/=0) then
