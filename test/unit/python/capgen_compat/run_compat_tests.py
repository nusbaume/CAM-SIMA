...
_CAPGEN = os.path.join(_CAM_ROOT, "ccpp_framework", "capgen")

# Same sys.path order test_write_init_files.py uses: compat shims
# (and their internal sibling modules) on the front; capgen itself
# appended.  All compat modules live as flat top-level imports.
sys.path.insert(0, _COMPAT)
if _CAPGEN not in sys.path:
    sys.path.append(_CAPGEN)
